...
<small>※ Java 8 채점기에서 해당 문구가 등장하는 경우입니다.
``` 컴파일 에러: 제출한 코드가 리트머스 서버에서 지원하는 제출 코드 형식에 해당하지 않습니다. ```
Java 8 코드의 일부 케이스에서는 상세한 컴파일 오류 메시지를 제공해주지 않습니다.
- public 최상위 타입이 2개 이상 존재할 때
- 주석 문자열에 ```class Main``` 패턴이 있을 때
